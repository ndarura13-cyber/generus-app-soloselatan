...
// Fetch pengurus dari Supabase saat module ini pertama kali dimuat (Top-Level Await)
try {
  const resPengurus = await fetchPengurusFromSupabase();
  if (resPengurus && resPengurus.success && resPengurus.data) {
    savePengurusList(resPengurus.data);
  }
} catch (error) {
  console.warn("Failed to load initial pengurus from Supabase:", error);
}
...
// Seluruh pengurus dikelola live langsung dari Supabase cloud (tidak menggunakan data lokal dummy)
const DEFAULT_PENGURUS_LIST = [];

// Helper Storage Pengurus
// Helper Storage Pengurus (Berbasis Live Supabase)
const PENGURUS_STORAGE_KEY = "ppg_registered_pengurus_v6_supabase";
...
export async function syncPengurusFromSupabase() {
  if (!isSupabaseConfigured()) return getPengurusList();
  try {
    ...
    console.warn("Gagal sinkronisasi pengurus dari Supabase:", err);
  }
  return getPengurusList();
}
...
export async function deletePengurus(pengurusId) {
  let list = getPengurusList();
  const idx = list.findIndex(p => p.id === pengurusId);
  if (idx === -1) return { success: false, message: "Data pengurus tidak ditemukan." };
  const nama = list[idx].nama || "Pengurus";
  list = list.filter(p => p.id !== pengurusId);
  savePengurusList(list);

  if (isSupabaseConfigured()) {
    try {
      await deletePengurusFromSupabase(pengurusId);
    } catch (err) {
      console.warn("deletePengurusFromSupabase error:", err);
    }
  }

  return { success: true, message: `Data pengurus "${nama}" berhasil dihapus.` };
}
...
export function addProker(data) {
  ...
  if (isSupabaseConfigured()) {
    upsertProkerToSupabase(newProker).catch(err => console.warn("Supabase addProker sync:", err));
  }
...
export function updateProker(id, updateData) {
  ...
  if (isSupabaseConfigured()) {
    upsertProkerToSupabase(list[idx]).catch(err => console.warn("Supabase updateProker sync:", err));
  }
...
export function deleteProker(id) {
  ...
  if (isSupabaseConfigured()) {
    deleteProkerFromSupabase(id).catch(err => console.warn("Supabase deleteProker sync:", err));
  }
...
export async function syncProkerFromSupabase() {
  ...
  try {
    ...
    console.warn("syncProkerFromSupabase error:", e);
  }
  return getProkerList();
}
